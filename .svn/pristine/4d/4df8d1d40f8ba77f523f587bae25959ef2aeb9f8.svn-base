...
/**
 * 改变现金券的奖项模式,如果模式为"按时段"的话则显示时间段添加的div
 * @param obj
 */
...
/**
 * 同步现金券金额隐藏表单域relateId
 * @param obj
 * @param relateId 关联的元素id
 */
function syncRedPrizeAmount(obj,relateId) {
	var redAmountVal = obj.value;
	if (validateMoney(redAmountVal)) {
		if(relateId!=""){
			$("#"+relateId).val(accMul(redAmountVal, 100));
		}
	}
}
/**
...
function saveCashCoupon() {
	...
	if(validatePrizeName()==false || validateTimingRed()==false || validateStartEndDate()==false
			...
			|| validatePrizeFactor()==false || validateValidDays()==false || validateCashGetAmount()==false
			|| validatePrizeRedHint()==false || validatePrizeDesc()==false){
...
function validatePrizeName(){
	var prizeName = $("#cash_prizeName").val();
	if ($.trim(prizeName)=="") {
		parent.alertMsg.error("请填写现金券奖品名称！");
		$("#cash_prizeName").focus();
		return false;
	} else if (prizeName.length > 20) {
		parent.alertMsg.error("现金券奖品名称长度不得超过20个字符！");
		$("#cash_prizeName").focus();
		return false;
	}
	return true;
...
function validateTimingRed() {
	...
	if ($.trim(redPrice) == "") {
		parent.alertMsg.error("请填写现金券金额！");
		$("#timingRed").focus();
		return false;
	}
	if (!reg.test(redPrice)) {
		parent.alertMsg.error("现金券金额格式不正确,例如" + example);
		$("#timingRed").focus();
		return false;
	} else if (redPrice > 200) {
		parent.alertMsg.error("现金券金额不得超过200");
		$("#timingRed").focus();
		return false;
	} else if (redPrice <= 0) {
		parent.alertMsg.error("现金券金额最低0.01元");
		$("#timingRed").focus();
		return false;
	}
	return true;
...
function validateStartEndDate(){
	...
	if($.trim(beginDate)==""){
		parent.alertMsg.error("请输入现金券开始日期");
		$("#beginDate").foucs();
		return false;
	}
	if($.trim(endDate)==""){
		parent.alertMsg.error("请输入现金券结束日期");
		$("#endDate").foucs();
		return false;
	}
	return true;
...
function validatePrizePeriodCount(){
	...
	if($.trim(prizePeriodCount)==""){
		parent.alertMsg.error("请填写现金券周期投放量");
...
function validatePrizeFactor(){
	...
	if($.trim(prizeFactor)==""){
		parent.alertMsg.error("请填写现金券中奖因子");
		$("#prizeFactor").focus();
		return false;
	}
	if(!isInteger(prizeFactor)||prizeFactor < 1){
		parent.alertMsg.error("现金券中奖因子必须填写大于等于1的整数");
		$("#prizeFactor").focus();
		return false;
	}
	return true;
...
/**
 * 验证有效天数,规则:>=0的整数,为0说明是无限制
 * @returns {Boolean}
 */
function validateValidDays(){
	var validDays = $("#validDaysId").val();
	if($.trim(validDays)==""){
		$("#validDaysId").focus();
		parent.alertMsg.error("请设置现金券有效天数");
		return false;
	}
	if(!isInteger(validDays)||validDays<0){
		$("#validDaysId").focus();
		parent.alertMsg.error("现金券有效天数应为大于等于0的整数");
		return false;
	}
	return true;
}
 ...
 * @returns {Boolean}
 */
function validateCashGetAmount(){
	var cashGetMinAmount = $("#cashGetMinAmountId").val();
	if($.trim(cashGetMinAmount)==""){
		$("#cashGetMinAmountId").focus();
		parent.alertMsg.error("请设置获得现金券的最大金额要求");
		return false;
	}
	var reg = /^\d+(\.\d{1,2}){0,1}$/;// 保留两位小数
	var example = "8.88或者100";
	if (!reg.test(cashGetMinAmount)) {
		$("#cashGetMinAmountId").focus();
		parent.alertMsg.error("获得现金券的最大金额格式有误,例如" + example);
		return false;
	} else if (cashGetMinAmount <= 0) {
		$("#cashGetMinAmountId").focus();
		parent.alertMsg.error("获得现金券的最大金额最低0.01元");
		return false;
	}
	return true;
}
