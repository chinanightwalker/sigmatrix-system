 ...
 (function($){
 	$.extend($.fn, {
		...
		_check:function(op) {
			...
			var tname = $input.attr("tname"), tvalue = $input.attr("tvalue"), fullname = $input.attr("fullName"), isresource = $input.attr("isResource");
			var attrs = "text='"+$input.text()+"' ";
			if (tname) attrs += "name='"+tname+"' ";
			if (tvalue) attrs += "value='"+tvalue+"' ";
			if (fullname) attrs += "fullname='"+fullname+"' ";
			if (isresource) attrs += "isresource='"+isresource+"' ";
