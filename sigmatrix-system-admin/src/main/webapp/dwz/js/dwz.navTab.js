...
var navTab = {
	...
	_switchTab: function(iTabIndex){
		...
		this._getMoreLi().removeClass("selected").eq(iTabIndex-1).addClass("selected");
	...
	_closeTab: function(index, openTabid){
		...
		this._getMoreLi().eq(index-1).remove();
	...
	_closeOtherTab: function(index){
		...
		if (index > 0) {
			...
			this._getMoreLi().not(str2$).remove();
	...
	checkTimeout:function(){
		...
		//openTabLs("productSeries","series!list.html","产品管理");
	...
	openTabLs: function(tabid, url, titleTxt, options){ //if found tabid replace tab, else create a new tab.
		var op = $.extend({title:titleTxt, data:{}, fresh:true, external:false}, options);
